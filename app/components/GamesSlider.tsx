...
function GamesSlider({
  ...
  return (
    <div className="mt-10 px-6">
      <h1
        className="text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 via-pink-400 to-purple-500 
                     text-3xl lg:text-5xl font-extrabold tracking-wide capitalize mb-8 ml-6"
      >
        {title}
      </h1>

      {/* ✅ Swiper مع autoplay و loop */}
      <Swiper
 spaceBetween={20}
  ...
  breakpoints={
    slidesPerView
      ? {
          ...
          640: { slidesPerView: slidesPerView }, // 💻 تابلت وفوق
        }
      : {
          0: { slidesPerView: 1.2 },
...
>
        ...
        {games.map((game, index) => {
          const imgSrc = game.background_image ?? placeholder;
          const key = String(game.id ?? index);
